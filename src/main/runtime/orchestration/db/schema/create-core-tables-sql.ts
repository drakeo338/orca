...
CREATE TABLE IF NOT EXISTS runs (
  ...
  -- Bare Orca session id the coordinator is addressed by (for a /clear'd chat, its lineage root's).
...
-- Handle-only on purpose; migrate-v42 replaces both triggers with a form that also remembers a
-- handle-less coordinator by its session address. This SQL runs before migrate on every open, so it
