...
// restart's own cause; the shared resume has already written that cause into the chat. A manual
// Retry or a new send is a fresh attempt: a refusal before admission leaves no ledger row behind.
//
...
import { structuredAgentSessionResumeRefusalOutcome } from './structured-agent-session-resume-refusal'
...
async function restartOwnerForSend(
  ...
  const outcome = resumed.ok ? null : structuredAgentSessionResumeRefusalOutcome(resumed.refusal)
  ...
  return ownerRestartFailedRefusal(record, resumed.refusal, outcome !== 'unresumable')
}
