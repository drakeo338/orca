...
export async function resumeHeldStructuredAgentSession(input: {
  ...
  /** Opens the journal once the lease is adjudicated, before the attach holds the queue for the
   *  whole provider start; a refusal ends the resume there. */
  openJournal?: (sessionId: string) => Promise<AgentSessionWireRefusal | null>
  ...
  const unopened = await input.openJournal?.(sessionId)
  if (unopened) {
    return { ok: false, refusal: unopened }
  }
